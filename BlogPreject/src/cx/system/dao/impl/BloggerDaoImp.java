...
public class BloggerDaoImp implements BloggerDao{
	...
	@Override
	public void updBlogger(Blogger blogger,String originBloggerName) {
		// TODO Auto-generated method stub
		String sql = "UPDATE blogger set name=?,password=?,likenumber=?,rewardnumber=?,headpicture=?,time=? WHERE name=?";  
        jdbcTemplateObject.update(sql,blogger.getName(),blogger.getPassword(),blogger.getLikeNumber(),blogger.getRewardNumber(),blogger.getHeadpicture(),blogger.getDatetime(),originBloggerName);  
        return ; 
	}
