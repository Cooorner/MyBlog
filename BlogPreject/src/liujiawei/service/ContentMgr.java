...
private static ContentMgr instance=null;
	...
	public Article getArticleByNumber(int number) {
		List<Article> list_Article=articleDaoImp.seaarticle(number);
		if(list_Article!=null&&list_Article.size()>0){
			return list_Article.get(0);
		}
		return null;
	}
	...
	public List<Article> getArticleByType(String type) {
		List<Article> list_Article=articleDaoImp.seaarticle2(type);
		if(list_Article==null){
			list_Article=new ArrayList<Article>();
		}
		return list_Article;
	}
	 ...
	 * 为文章设置点赞量
	 */
	public void setArticleLikeNumber(int articlenumber,int likenumber) {
		articleDaoImp.addLikeNumber(articlenumber, likenumber);
	}
